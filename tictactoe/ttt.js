...
function markSquare(clickedSquare) {
    ...
    if (!gameOver) {
        if (clickedSquare.innerHTML !== `-`) {
            ...
            whosTurn = 2;
            removingBoxFromSelection(player1Squares, clickedSquare);
            checkWin(1, player1Squares);
            //Computer's selection
            if (x.style.display === "none" && whosTurn === 2) {
                // let randPick = function computerPick() {
                //     return Math.floor(Math.random() * computerSelection.length - 1)
                // };
                var randPick = null;

                if (computerSelection[randPick()] == null) {
                    var randPick = function computerPick() {
                        return Math.floor(Math.random() * computerSelection.length - 1)
                    };
                } else {
                    document.getElementById(computerSelection[randPick()]).click();
                }
                //
            ...
            whosTurn = 1;
            removingBoxFromSelection(player2Squares, clickedSquare);
...
function removingBoxFromSelection(array, clickedSquare) {
    array.push(clickedSquare.id);
    const sqIndex = computerSelection.indexOf(clickedSquare.id);
    computerSelection.splice(sqIndex, 1);
}
